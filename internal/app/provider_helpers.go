...
func resolveProviderURL(baseURL, reference string) string {
	reference = strings.TrimSpace(html.UnescapeString(reference))
	if reference == "" {
		return ""
	}
...
func sortProviderChapters(chapters []Chapter) {
	sort.SliceStable(chapters, func(left, right int) bool {
		leftEpisode, leftErr := strconv.Atoi(chapters[left].EpisodeString(left + 1))
		rightEpisode, rightErr := strconv.Atoi(chapters[right].EpisodeString(right + 1))
		if leftErr == nil && rightErr == nil && leftEpisode != rightEpisode {
			return leftEpisode < rightEpisode
		}
		return chapters[left].ID < chapters[right].ID
	})
}
