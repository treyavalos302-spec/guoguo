...
func (a *UIApp) ListenAndServe(addr string) error {
	...
	if _, err := a.ensureExternalPaths(); err != nil {
		return fmt.Errorf("私密入口初始化失败：%w", err)
	}
	if _, err := a.forwardAccessToken(); err != nil {
		return fmt.Errorf("Forward 访问令牌初始化失败：%w", err)
	}
...
func (a *UIApp) routes() http.Handler {
	...
	if paths, err := a.ensureExternalPaths(); err == nil {
		mux.HandleFunc(paths.Emby+"/stream.m3u8", a.handleEmbyStream)
		mux.HandleFunc(paths.Emby+"/segment.ts", a.handleEmbySegment)
		mux.HandleFunc(paths.Forward+"/", a.handleForward)
	}
