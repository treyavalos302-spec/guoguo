...
func TestEmbyExportPersistentSignedLinksAndSafeArchive(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "http://localhost/api/ui/admin/emby/export", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	result := httptest.NewRecorder()
	app.handleEmbyExport(result, req)
...
func TestEmbyHLSRangePlaybackAndExpiry(t *testing.T) {
	...
	writer := httptest.NewRecorder()
	app.handleEmbyStream(writer, httptest.NewRequest(http.MethodGet, "http://localhost"+paths.Emby+"/stream.m3u8?"+query.Encode(), nil))
	...
	partURL := "http://localhost" + paths.Emby + "/" + segment
	...
	defer cancel()
	output, err := exec.CommandContext(ctx, probe, "-v", "error", "-show_entries", "format=duration", "-of", "default=nw=1:nk=1", server.URL+paths.Emby+"/stream.m3u8?"+query.Encode()).CombinedOutput()
