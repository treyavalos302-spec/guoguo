...
func playbackNativeSegmentCount(duration float64) int {
	if duration <= 0 || math.IsNaN(duration) || math.IsInf(duration, 0) || duration > 24*60*60 {
		return 0
	}
	whole := int(math.Floor(duration / playbackNativeSegmentSeconds))
	if whole > 0 && duration-float64(whole*playbackNativeSegmentSeconds) <= 0.05 {
		return whole
	}
	return whole + 1
}

func playbackNativeSegmentDuration(duration float64, index int) float64 {
	count := playbackNativeSegmentCount(duration)
	if index < 0 || index >= count {
		return 0
	}
	if index == count-1 {
		return math.Min(playbackNativeSegmentSeconds, math.Max(0.001, duration-float64(index*playbackNativeSegmentSeconds)))
	}
	return playbackNativeSegmentSeconds
}

func (cache *playbackNative) segmentCountLocked() int {
	return playbackNativeSegmentCount(cache.duration)
}
...
func (cache *playbackNative) cachedSegment(index int) []byte {
	cache.mu.Lock()
