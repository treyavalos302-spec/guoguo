...
func (app *UIApp) embySigningKey(create bool) ([]byte, error) {
	...
	keyPath := filepath.Join(app.cfg.dataDirectory(), "emby-key")
	readKey := func() ([]byte, error) {
		...
		if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
			return nil, errors.New("Emby 链接密钥必须是普通文件")
		}
...
func embyRequestBaseURL(request *http.Request, supplied string) (string, error) {
	if configured := strings.TrimSpace(os.Getenv("JUKU_PUBLIC_URL")); configured != "" {
		return embyBaseURL(configured)
	}
	if strings.TrimSpace(supplied) != "" {
		return embyBaseURL(supplied)
	}
	scheme := "http"
	if request.TLS != nil || strings.EqualFold(request.Header.Get("X-Forwarded-Proto"), "https") {
		scheme = "https"
	}
	return embyBaseURL(scheme + "://" + request.Host)
}
...
func (app *UIApp) handleEmbyExport(writer http.ResponseWriter, request *http.Request) {
	...
	body, err := buildEmbyArchive(drama, chapters, base, paths.Emby, key, owner)
	if err != nil {
...
func buildEmbyArchive(drama Drama, chapters []Chapter, base, embyPath string, key []byte, accounts ...string) ([]byte, error) {
	owner := ""
	if len(accounts) > 0 {
		owner = accounts[0]
	}
	...
	for index, chapter := range chapters {
		...
		if err = write(path+".strm", []byte(base+embyPath+"/stream.m3u8?"+query.Encode()+"\n")); err != nil {
			return nil, err
		}
		episode := struct {
