...
func (app *UIApp) withAccountAccess(next http.Handler) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if strings.HasPrefix(path, "/assets/") || app.externalPublicPath(path) {
...
func (app *UIApp) handleAdminSettings(writer http.ResponseWriter, request *http.Request) {
	...
	if request.Method == http.MethodGet {
		...
		response := map[string]any{"requireLogin": requireLogin, "allowRegistration": allowRegistration}
		if paths, err := app.ensureExternalPaths(); err == nil {
			response["forwardPath"], response["embyPath"] = paths.Forward, paths.Emby
			response["forwardModulePath"] = paths.Forward + "/module.js"
		}
		if token, err := app.forwardAccessToken(); err == nil {
			response["forwardToken"] = string(token)
		}
		response["embyExportDirectory"] = strings.TrimSpace(os.Getenv("JUKU_EMBY_EXPORT_DIR"))
