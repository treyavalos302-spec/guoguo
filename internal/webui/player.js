(() => {
  ...
  const nativeRecovery = capabilities.recovery();
  ...
  async function requestJSON(path, body, signal, timeoutMS = 100000) {
    ...
    try {
      ...
      if (timedOut) {
        const timeoutError = new Error('播放请求超时，请检查连接后重试');
        timeoutError.playbackTimeout = true;
        throw timeoutError;
      }
      throw error;
    } finally {
  ...
  function waitForEvent(target, eventName, signal, action, timeoutMS = 0, timeoutMessage = '视频加载超时') {
    return new Promise((resolve, reject) => {
      ...
      function timedOut() {cleanup(); const error = new Error(timeoutMessage); error.playbackTimeout = true; reject(error);}
      if (signal.aborted) {aborted(); return;}
      target.addEventListener(eventName, done, {once: true});
      target.addEventListener('error', failed, {once: true});
      signal.addEventListener('abort', aborted, {once: true});
      if (timeoutMS > 0) deadline = setTimeout(timedOut, timeoutMS);
  ...
  function handleNativeTimeout(message) {
    if (transport !== 'hls' || !panel.open || !currentIndex) return false;
    if (nativeRecovery.retry(currentIndex)) {
      statusText.textContent = '播放响应过慢，正在自动重建播放流…';
      playEpisode(currentIndex, recoveryPosition(), desiredPlayback, true, true);
      return true;
    }
    streamController?.abort();
    showError(new Error('HLS 播放自动重建后仍失败：' + message));
    return true;
  }

  function scheduleNativeWatchdog(message, delay = 30000) {
    if (nativeWatchdogTimer || transport !== 'hls' || !panel.open || !currentIndex) return;
    const index = currentIndex;
    const version = streamVersion;
    nativeWatchdogTimer = setTimeout(() => {
      nativeWatchdogTimer = null;
      if (transport === 'hls' && panel.open && index === currentIndex && version === streamVersion) handleNativeTimeout(message);
    }, delay);
  }
  ...
  async function playNative(index, offset, shouldPlay, version, currentSession, signal) {
    nativeRecovery.start(index);
    statusText.textContent = '正在生成首个播放分片…';
    const result = await requestJSON('/api/ui/playback/hls/open', {session: currentSession, episode: index, start: offset, quality, version}, signal, 35000);
    ...
    statusText.textContent = result.prefetched ? '正在读取预缓存…' : '正在加载播放列表…';
    await waitForEvent(video, 'loadedmetadata', signal, () => {video.src = result.url; video.load();}, 20000, '浏览器未能读取播放列表，请检查网络或内置浏览器权限');
    if (signal.aborted || version !== streamVersion) throw abortError();
    ...
    qualitySelect.disabled = false;
    mobile.update();
    statusText.textContent = shouldPlay ? '正在等待第一帧…' : '已暂停';
    if (!shouldPlay) {
      clearNativeWatchdog();
      return;
    }
    scheduleNativeWatchdog('浏览器未能开始播放，请检查网络后重试', 20000);
    video.play().catch(error => {
      if (version !== streamVersion || signal.aborted) return;
      if (error.name === 'NotAllowedError') {
        clearNativeWatchdog();
        statusText.textContent = '已就绪，点击视频中的播放按钮';
      } else if (error.name !== 'AbortError') handleNativeTimeout(error.message || '浏览器未能开始播放');
    });
  }

  async function playEpisode(index, offset = 0, shouldPlay = true, keepResumeMessage = false, nativeRebuild = false) {
    if (!episodes[index - 1]) return;
    if (transport === 'hls' && !nativeRebuild) nativeRecovery.reset();
    ...
    try {
      ...
      if (version === streamVersion && !signal.aborted) {
        ...
        if (transport === 'hls' && handleNativeTimeout(error.message || '播放流加载失败')) return;
  ...
  video.addEventListener('playing', () => {nativeWaiting = false; clearNativeWatchdog(); desiredPlayback = true; if (!loading && !errorText.textContent) {historyPlayed = true; historyCompleted = false; statusText.textContent = '正在播放'; saveHistory(true);} maybePrefetchNext();});
  for (const eventName of ['loadeddata', 'canplay', 'progress']) video.addEventListener(eventName, () => {if (transport === 'hls' && nativeWaiting && !video.paused) scheduleNativeWatchdog('视频分片长时间没有继续加载，请检查网络后重试');});
  for (const eventName of ['waiting', 'stalled']) video.addEventListener(eventName, () => {if (loading || errorText.textContent || transport !== 'hls' || video.paused) return; nativeWaiting = true; statusText.textContent = '正在缓冲…'; scheduleNativeWatchdog('视频持续缓冲，请检查网络或内置浏览器权限');});
  video.addEventListener('pause', () => {nativeWaiting = false; clearNativeWatchdog(); if (!loading && !video.error) desiredPlayback = false; if (!loading) saveHistory(true); if (!loading && !video.ended && !errorText.textContent) statusText.textContent = '已暂停';});
  video.addEventListener('error', async () => {
    ...
    if (transport === 'hls' && handleNativeTimeout('浏览器播放失败（错误 ' + code + '）')) return;
  ...
  video.addEventListener('play', () => {
    if (loading) return;
    if (transport === 'hls') scheduleNativeWatchdog('浏览器未能开始播放，请检查网络后重试', 20000);
    wakePlayback();
  });
