...
export function createDetails(app) {
  ...
  function render() {
    ...
    withFocus(content, () => {
      ...
      const sync = button(syncing.has(currentID) ? '同步中…' : '同步 Emby', () => syncEmby(currentID), !known || syncing.has(currentID), 'secondary');
      sync.id = 'detailEmbySyncBtn';
      sync.dataset.focusKey = 'detail-emby-sync';
      sync.title = '安全同步到服务端 JUKU_EMBY_EXPORT_DIR，不删除已有无关文件';
      if (app.viewer?.onlineOnly) secondary.appendChild(watched);
      else if (app.viewer?.account?.admin) secondary.append(app.downloads.qualityControl(), download, watched, emby, sync);
      else secondary.append(app.downloads.qualityControl(), download, watched);
  ...
  async function syncEmby(id) {
    if (syncing.has(id)) return;
    syncing.add(id);
    render();
    try {
      const result = await app.post('/api/ui/admin/emby/sync', {dramaId: id, baseUrl: location.origin});
      setMessage('Emby 同步完成：' + result.episodes + ' 集，写入 ' + result.written + ' 个文件，未变化 ' + result.unchanged + ' 个。');
    } catch (error) {setMessage('Emby 同步失败：' + error.message, true);}
    finally {syncing.delete(id); if (panel.open && currentID === id) render();}
  }
